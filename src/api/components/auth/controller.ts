...
export = () => {
    const upsert = async (body: IAuth, email: string, name: string) => {
        ...
        if (body.pass) {
            ...
            if (result.dataValues.id) {
                return await sendPass(name, body.user, newPass, email, "Nuevo usuario", true, false);
            } else {
                return false;
            }
        }
    ...
    const recPass = async (email: string) => {
        ...
        return await upsert(data, email, `${userData[0].dataValues.name} ${userData[0].dataValues.lastname}`);
    }
